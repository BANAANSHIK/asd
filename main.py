...
masseanemies = ['троль', 'скелет', 'паук', 'мимик', 'демон', 'бронировый дух', {
    "name":"троль",
    "damage": 5,
    "hp": 20,
    "armor": 2
...
# enemy["hp"] = 21
# print(enemy["hp"])
# для словарика если он нужен

# print(masseanemies[0])
# print(len(masseanemies)-1)
# print(masseanemies[len(masseanemies)-1])
# masseanemies.append('129')
# print(masseanemies)
# masseanemies.remove('129')
# название = значение
...
while isGame:
    ...
    rand = random.randint(0,len(masseanemies)+10)


    if len(masseanemies)>rand and masseanemies[rand] == 'троль':
        enemyname = masseanemies[rand]
        hpenemy = masseanemies[rand]['hp']
        damageenemy = masseanemies[rand]['damage']
        hpenemy = 25
        damageenemy = 5

        while True:
            print(f"передвами вылез {enemyname}, что будем делать.")
            choice = input("1. Атаковать 2. Защититься 3. увернуться")
            if choice == "1":
                hpenemy -= damage
                hp -= damageenemy // 1.5
                print(f"{enemyname} получил бабам,у него осталось {hpenemy} hp")
                print(f"но и тебе прилетело {damageenemy // 1.5} урона\n")
            elif choice == "2":
                if damageenemy > defense:
                    hp = hp - (damageenemy - defense)
                    print(f"{enemyname}  пробил вашу защиту он нанёс {damageenemy - damage} урона")
                else:
                    print('cool defens')
            elif choice == "3":
